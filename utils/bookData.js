...
export const STORAGE_KEYS = {
  ...
  BOOK_PROGRESS: 'book_progress_' // 会加上书籍ID
}

// 获取当前选中的书籍
export function getCurrentBook() {
  try {
    const bookId = uni.getStorageSync(STORAGE_KEYS.CURRENT_BOOK) || 1
...
export function setCurrentBook(bookId) {
  try {
    uni.setStorageSync(STORAGE_KEYS.CURRENT_BOOK, bookId)
  } catch (e) {
...
export function getBookProgress(bookId) {
  try {
    const progress = uni.getStorageSync(STORAGE_KEYS.BOOK_PROGRESS + bookId) || {
      learnedWords: [],
      wrongWords: [],
      reviewWords: []
    }
...
export function saveBookProgress(bookId, progress) {
  try {
    uni.setStorageSync(STORAGE_KEYS.BOOK_PROGRESS + bookId, progress)
  } catch (e) {
...
export function getCurrentBookWrongWords() {
  ...
  return progress.wrongWords.map(id => 
    bookWordList.find(word => word.id === id)
  ).filter(Boolean)
}
...
export function getCurrentBookLearnedWords() {
  ...
  return progress.learnedWords.map(id => 
    bookWordList.find(word => word.id === id)
  ).filter(Boolean)
}
